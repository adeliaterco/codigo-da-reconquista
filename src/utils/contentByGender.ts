...
export function getCopy(quizData: QuizData): string {
  ...
  const action = quizData.gender === 'HOMBRE' ? 'conquistarla' : 'conquistarlo';
  const needed = quizData.gender === 'HOMBRE' ? 'la mujer que él necesitaba' : 'el hombre que ella necesitaba';
  
  return `No fue por falta de amor.

Fue porque en algún momento dejaste de ser ${needed}.

Pero aquí está la verdad: eso puede cambiar.

Y en el siguiente paso, voy a revelar EXACTAMENTE qué fue lo que cambió y el paso a paso para que ${pronoun} sienta que SÍ eres suficiente.`;
}
...
export function getCompletionBadge(gender: string): { title: string; subtitle: string } {
  ...
  return {
    ...
    subtitle: `Descubre exactamente por qué ${pronoun} se fue y el paso a paso para que ${pronoun} QUIERA volver`
  };
}
